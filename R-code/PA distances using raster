...
PA_xy <- as.data.frame(PA_r, xy=T, na.rm=T)
PA_xy_mm <- aggregate(cbind(x, y) ~ WDPA_IDAreaWeightedIntersectTerr_r16d, PA_xy, function(XX) {c(min=min(XX) , max=max(XX) )})
...
for (i in IDS) {
  ...
  XY_i=unlist((as.vector(PA_xy_mm[PA_xy_mm$WDPA_IDAreaWeightedIntersectTerr_r16d==i, 2:3])))
...
write.table(data.frame(Crop, list_length, PA_N, TIME2, SIZE), file="con_raster_distance_times3.csv", append=T, row.names=F)

#write results
write.table(PA_edge_dist, file=paste("C://HY-Data/KULLBERG/GPAN_2015_DATA/Distances/Dist_tables/", "PA_rast_dist_area_", Crop, "windowS_", SIZE, ".csv", sep=""), append=F, sep="\t", row.names=F)
