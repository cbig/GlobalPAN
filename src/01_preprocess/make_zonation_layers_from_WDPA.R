...
# make a hierarchical mask that removes PAs the last
...
writeRaster(WDPA_hier_mask, "../data/masks/wdpa_hier_mask.tif",
            format="GTiff", datatype="INT4S", options="COMPRESS=DEFLATE",
            overwrite=T)

# IDs have to be reclassified for the PLU mask because some of the codes
# are too large for zonation to handle  (ones starting with 555555...)
WDPA_plu_work <- WDPA_mask
WDPA_plu_work[WDPA_hier_mask==0] <- 1 # This can be done as there isn't PA with ID 1
